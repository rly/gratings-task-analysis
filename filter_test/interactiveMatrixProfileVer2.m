...
excZoneLen = round(subLen * 0.9);
radius = 5;
updatePeriod = 5; % in second
...
%                 motifIdxs{j, 1}(1), motifIdxs{j, 1}(2)));
            motifPlot{j, 1}(k) = plot(1:subLen, ...
                ...
                motifColor{k}, 'linewidth', 2, 'parent', mainWindow.motifAx(j));
            hold(mainWindow.motifAx(j), 'off');
        end
    end
...
for i = 1:3
    ...
    maxNeighbors = 30;
    motifNeighbor = zeros(1, maxNeighbors); % max 10 neighbors
    for j = 1:maxNeighbors
