...
for uniti = 1:nUnits
    ...
    if ~isempty(cueOnsetTrialInRF)
        ...
        if ~isempty(cueOnsetTrialOutRF)
            cueOnsetTrialOutRF = UE.cueOnset(UE.cueLoc == 1);%UE.cueOnset(UE.isHoldTrial & UE.cueLoc == 4);
        ...
        elseif isempty(cueOnsetTrialOutRF)
            cueOnsetTrialOutRF = UE.cueOnset(UE.cueLoc == 1);%UE.cueOnset(UE.isHoldTrial & UE.cueLoc == 4);
            for cuei = 1:numel(cueOnsetTrialOutRF)
                nwinr = cueOnsetTrialOutRF(cuei);
                nwinl = cueOnsetTrialOutRF(cuei) - 0.175;
        ...
        spikesInTrialsAttIn = zeros(numel(arrayOnsetTrialAttIn),300); spikesInTrialsAttOut = zeros(numel(arrayOnsetTrialAttOut),300);
        isiAttOut = []; isiAttIn = [];
        ...
        if length(pSCCOsum) == length(pSCCIsum)
            if (pSCCIsum(uniti) <= .05 && meanSCInRFCount(uniti) > meanSCInRFBlCount(uniti)) || (pSCCOsum(uniti) <= .05 && meanSCOutRFCount(uniti) > meanSCOutRFBlCount(uniti))
                ...
                idx = 1;
                for cuei = 1:numel(arrayOnsetTrialAttIn)
                    %                 nwinr = arrayOnsetTrialAttIn(cuei);
                    nwinl = arrayOnsetTrialAttIn(cuei);
%                     nwinr = targetDimTrialAttIn(cuei);
                    nwinr = nwinl + 0.650; % earliest target at 650ms
                ...
                for cuei = 1:numel(arrayOnsetTrialAttOut)
                    %                 nwinr = arrayOnsetTrialAttOut(cuei);
                    nwinl = arrayOnsetTrialAttOut(cuei);
                    nwinr = nwinl + 0.650; % earliest target at 650ms
%                     nwinr = targetDimTrialAttOut(cuei);
                ...
                condiInfo(uniti,:) = [numel(arrayOnsetTrialAttIn) numel(arrayOnsetTrialAttOut) sum(spikeCountIn) sum(spikeCountOut)];
                spikeFs = 1000;
                ...
                if size(spikesInTrialsAttIn,2) ~= size(spikesInTrialsAttOut,2)
                    if size(spikesInTrialsAttIn,2) > size(spikesInTrialsAttOut,2)
                        spikesInTrialsAttOut(:,size(spikesInTrialsAttOut,2):size(spikesInTrialsAttIn,2)) = 0;
                    elseif size(spikesInTrialsAttOut,2) > size(spikesInTrialsAttIn,2)
                        spikesInTrialsAttIn(:,size(spikesInTrialsAttIn,2):size(spikesInTrialsAttOut,2)) = 0;
                    end
                end
...
title('BRI over AT 0-650ms')

for uni = 1:length(allISIAttIn)
    attInISIperc(uni) = (sum(allISIAttIn{uni}<=5)*100) / length(allISIAttIn{uni});
    attOutISIperc(uni) = (sum(allISIAttOut{uni}<=5)*100) / length(allISIAttOut{uni});
    attISIperc(uni) = (sum(allISIAttIn{uni}<=5) + sum(allISIAttOut{uni}<=5))*100 / (length(allISIAttIn{uni}) + length(allISIAttOut{uni}));
end
figure
subplot(211)
histogram(attInISIperc(~isnan(attInISIperc)),linspace(0,60,61),'FaceColor',[0.5 0.5 0.5])
ylabel('Number of neurons')
title('Examining bimodality in short-ISIs in AT')
subplot(212)
histogram(attOutISIperc(~isnan(attOutISIperc)),linspace(0,60,61))
hold on
histogram(attInISIperc(~isnan(attInISIperc)),linspace(0,60,61))
ylabel('Number of neurons')
legend({'AttOut' 'AttIn'})
xlabel('Percentage of ISIs <5ms (%)')
