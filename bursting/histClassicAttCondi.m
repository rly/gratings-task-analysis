function [firingRatefun, percClassicTwoSpikeBurstfun, percClassicThreeSpikeBurstfun, numInterSpikeTimesfun, numBurstLongfun,...
    ...
    cueOnset,arrayOnset,targetDimOnset] = histClassicAttCondi(spikeTimes2use, cueOnsetTrialAttIn, cueOnsetTrialAttOut,...
...
if tempResolve
    ...
    cueOnset = createTimeLockedSpdf(spikeTimes2use(quietInd(burstInd)+1), UE.cueOnset, UE.cueOnsetByLoc, cueOnset, kernelSigma, startTime, endTime);
...
for n = 1:numel(isiPerTrialAttIn)
    qPeriod = isiPerTrialAttIn(n).isi>maxTime;
    if sum(qPeriod)>=1
        if sum(find(qPeriod)+1<=numel(isiPerTrialAttIn(n).isi)) < sum(qPeriod)
            qPeriod = qPeriod(1:end-1);
        end
        if isiPerTrialAttIn(n).isi(find(qPeriod)+1) <= minTime
...
for n = 1:numel(isiPerTrialAttOut)
    qPeriod = isiPerTrialAttOut(n).isi>maxTime;
    if sum(qPeriod)>=1
        if sum(find(qPeriod)+1<=numel(isiPerTrialAttOut(n).isi)) < sum(qPeriod)
            qPeriod = qPeriod(1:end-1);
        end
        if isiPerTrialAttOut(n).isi(find(qPeriod)+1) <= minTime
