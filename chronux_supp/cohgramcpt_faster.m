function [C,phi,S12,S1,S2,t,f,zerosp,confC,phistd,Cerr]=cohgramcpt_faster(data1,data2,movingwin,params,fscorr)
...
for n=1:nw;
   ...
   t=indx/Fs;
   datawin1=data1(indx,:);datawin2=extractdatapt_faster(data2,[indx(1)/Fs indx(end)/Fs]);
   if nargout==11;
     [c,ph,s12,s1,s2,f,zsp,confc,phie,cerr]=coherencycpt_faster(datawin1,datawin2,params,fscorr,t);
...
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
   ...
   elseif nargout==10;
     [c,ph,s12,s1,s2,f,zsp,confc,phie]=coherencycpt_faster(datawin1,datawin2,params,fscorr,t); 
%      phierr(1,n,:,:)=squeeze(phie(1,:,:));
%      phierr(2,n,:,:)=squeeze(phie(2,:,:));
     phistd(n,:,:)=phie;
   else
     [c,ph,s12,s1,s2,f,zsp]=coherencycpt_faster(datawin1,datawin2,params,fscorr,t);
   end;
   C(n,:,:)=c;
   phi(n,:,:)=ph;
   S12(n,:,:)=s12;
