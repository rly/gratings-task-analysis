function [C,phi,S12,S1,S2,t,f,zerosp,confC,phistd,Cerr]=cohgramcptjitter(data1,data2,movingwin,params,fscorr,absMaxJitter)
% Multi-taper time-frequency coherence,cross-spectrum and individual spectra
%   continuous process and point process times with random jitter applied
%   to spike times within each window
%
% Usage:
%
...
for n=1:nw;
   ...
   t=indx/Fs;
   datawin1=data1(indx,:);
   datawin2orig=extractdatapt(data2,[indx(1)/Fs indx(end)/Fs]);
   % this is going to be super slow
   datawin2 = datawin2orig;
   for i = 1:numel(datawin2orig)
       datawin2(i).times = addBoundedRandom(datawin2(i).times, absMaxJitter, [indx(1)/Fs indx(end)/Fs]);
   end
   if nargout==11;
