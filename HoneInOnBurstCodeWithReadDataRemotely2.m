...
%                 end
                ...
                percentageBurstWdMeth =  sum(diff(spikeTimes2use)<(1/200)) * 100 / size(spikeTimes2use,2);

                spikeTimes2use = spikeTimes2use';
                allDiffSpikeTimes = diff(spikeTimes2use); % get the ISIs
                ...
                firstSpikeBurstLib = sum(allDiffSpikeTimesPre > log10(50) & allDiffSpikeTimesPost < log10(10),1);
                overFirstSpikeBurstLib = sum(allDiffSpikeTimesPre < log10(10),1);
                burstISIsLib = overFirstSpikeBurstLib + firstSpikeBurstLib;
                libPercBurst(uniti) = (burstISIsLib*100)/allISIs;
                ...
                subplot(233)
                histogram(10.^allDiffSpikeTimesPre,'BinWidth',10)%,10.^timeXaxis)
                xlim([0 200])
                xlabel('ISI in ms')
                title('Histogram ISI')
...
axis off
                text(-0.2,0.5, {...
                        sprintf('Conservative (4ms + 100ms) burst %%: %0.2f', percBurst(uniti)), ...
                        sprintf('Liberal (10ms + 50ms) burst %%: %0.2f', libPercBurst(uniti)), ...
                        sprintf('Womelsdorf method (200Hz) burst %%: %0.2f', percentageBurstWdMeth)});
                ...
                plotFileName = sprintf('%s/%s-sessionInd%d-sessioni%d-Uniti%d-BurstingTvV2-v%d.png', outputDir, unitStruct.name,sessionInd, sessioni, uniti, v);
