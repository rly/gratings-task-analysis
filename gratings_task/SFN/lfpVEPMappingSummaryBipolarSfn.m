...
for s = 1:nSessionsIncl
    ...
    newY = (1+minShift):(31+maxShift);
    nChannelsNew = numel(newY);
    shiftResponse = nan(nChannelsNew, nTime);
    [~,newYInd] = intersect(newY, (1:31)+shift);
    shiftResponse(newYInd,:) = meanResponseAll{sessionIndsIncl(so)}(2:32,:) - meanResponseAll{sessionIndsIncl(so)}(1:31,:);  
...
plotFileName = sprintf('%s/SFN/ex-lfp-vepm-alignment-v%d.png', processedDataRootDir, v);
