...
dPulPower = nan(10, nFreq);
vPulPower = nan(10, nFreq);
dPulCount = 0;
vPulCount = 0;
...
for i = 1:nSessions
    ...
    % channel x time
    meanResponseAll{i} = nan(size(RR.responses, 1), nFreq);
    
    % get mean response across flashes
    for j = 1:numel(lfpChannelsToLoad)
        baselineInd = t < 0;
        [S,f] = mtspectrumc(squeeze(RR.responses(j,baselineInd,:)), params);
        meanResponseAll{i}(j,:) = S;
        
        if any(j == R.dPulChannels)
            dPulCount = dPulCount + 1;
            dPulPower(dPulCount,:) = S;
        elseif any(j == R.vPulChannels)
            vPulCount = vPulCount + 1;
            vPulPower(vPulCount,:) = S;
        end
    end
end
...
commonAverageAll = nan(nSessions, nFreq);
for i = 1:nSessions
...
figure;
hold on;
plot(f, mean(10*log10(dPulPower)));
plot(f, mean(10*log10(dPulPower)) - std(10*log10(dPulPower)) / sqrt(size(dPulPower, 1)));
plot(f, mean(10*log10(vPulPower)));
plot(f, mean(10*log10(vPulPower)) + std(10*log10(vPulPower)) / sqrt(size(vPulPower, 1)));
xlim([8 70]);

%%
figure;
hold on;
plot(f, (10*log10(dPulPower)));
xlim([8 70]);
...
for s = 1:nSessionsIncl  
    ...
    shiftResponse = nan(nChannelsNew, nFreq);
    [~,newYInd] = intersect(newY, (1:nChannels)+shift);
    shiftResponse(newYInd,:) = 10*log10(meanResponseAll{sessionIndsIncl(so)});
