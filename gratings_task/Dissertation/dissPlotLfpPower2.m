function dissPlotLfpPower2(spectraLF, spectraHF, fAxisLF, fAxisHF, conds, varargin)
...
plotLFW = diff(xBounds(1,:)) / 85;
plotHFW = diff(xBounds(2,:)) / 85;
plotH = 0.75;
...
for i = 1:nCond
    ...
    plot(fAxisLF, spectraLF(conds(:,i),:), 'Color', cols(i,:));
...
for i = 1:nCond
    ...
    plot(fAxisHF, spectraHF(conds(:,i),:), 'Color', cols(i,:));
    legendLabels{i} = sprintf(' %s (N=%d)', lineLabels{i}, sum(conds(:,i)));
end
