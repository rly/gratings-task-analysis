function process_sfc_m20170127(processedDataDir, blockName, spikeStruct, ...
...
filePrefix = sprintf('%s-%s-%s', spikeVarName, lfpVarName, blockName);
...
for i = 1:nLoc
    ...
    lfpAroundTargetDimShortHoldByLoc{i} = createdatamatc(adjLfp, targetDimShortHoldByLoc{i}, Fs, periTargetDimWindow);
    lfpAroundTargetDimLongHoldByLoc{i} = createdatamatc(adjLfp, targetDimLongHoldByLoc{i}, Fs, periTargetDimWindow);
end
...
%% plot
plotFileName = sprintf('%s/%s-sfc.png', processedDataDir, filePrefix);
plotSessionCueArrDimSpikeFieldCoherence(blockName, ..., % TODO add sessionName
