...
%    %                                                   %
%    %     [R,t,E] = psth(data,sig,plt,T,err,t)  ;       %
%    %                                                   %
%    %  **** INPUT ****                                  % 
%    %                                                   %
...
function [R,t,E] = edpsth_notranspose(data,sig,plt,T,err,t)
if isempty(data), R = NaN; t = NaN; E = NaN; return; end %! RYAN PUT THIS IN. 
[data]=padNaN(data); % create a zero padded data matrix from input structural array %! RYAN PUT THIS IN. 
if all(isnan(data)), R = NaN; t = NaN; E = NaN; return; end %! RYAN PUT THIS IN. 
...
% if sz(1)>sz(2); data=data';end %! RYAN COMMENTED THIS OUT. 
data = data'; %! RYAN PUT THIS IN. 
if nargin < 2; sig = 0.05;end
if nargin < 3; plt = 'r';end
% if nargin < 5; err = 0; end % err = 2; %! RYAN COMMENTED THIS OUT. 
if nargin < 5; err = 2; end %! RYAN PUT THIS IN. 

if isempty(sig); sig = 0.05;end
if isempty(plt); plt = 'r';end
if isempty(err); err = 0; end %err = 2;
...
% if nargin < 4; %! RYAN COMMENTED THIS OUT. 
if nargin < 4 || isempty(T) %! RYAN PUT THIS IN. 
  T(1) = min(data(:,1));
  T(2) = max(max(data));
else
  T(1) = T(1)-4*sig;
...
% axis([T(1)+(4*sig) T(2)-(4*sig) 0 max(R+2*E)+10]) %uncomment when err = 2
xlabel('time (s)')
ylabel('rate (Hz)')
