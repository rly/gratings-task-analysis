...
%%
trialStructs = loadjson([rfmResultsRoot rfmResultsFiles{3}]);
...
driftRates = cell2mat({flashParams.driftRate});
...
driftRatesUnique = sort(unique(driftRates)); % degrees

allPsths = nan(numel(coordinatesUnique), numel(gratingAnglesUnique), numel(driftRatesUnique), nTime);
trialCounts = zeros(numel(coordinatesUnique), numel(gratingAnglesUnique), numel(driftRatesUnique));
responseMetric = zeros(numel(coordinatesUnique), numel(gratingAnglesUnique), numel(driftRatesUnique));
baselineMetric = zeros(numel(coordinatesUnique), numel(gratingAnglesUnique), numel(driftRatesUnique));

for k = 1:numel(coordinatesUnique)
    for l = 1:numel(gratingAnglesUnique)
        for m = 1:numel(numel(driftRatesUnique))
            theseFlashOnsets = flashOnsets(coordinatesIndex' == k & gratingAngles == gratingAnglesUnique(l) & driftRates == driftRatesUnique(m));
...
for k = 1:numel(coordinatesUnique)
    ...
    for l = 1:numel(gratingAnglesUnique)
        for m = 1:numel(driftRatesUnique)
            count1 = count1 + channelSep;
            plot(t, squeeze(allPsths(k,l,m,:))*yScale + count1, 'Color', cols(m,:));
        end
