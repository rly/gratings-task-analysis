...
adjLfpsCAR = D.adjLfps - nanmean(D.adjLfps, 1);
% D.adjLfps = []; % clear memory

% remove all starting nans and adjust the event times accordingly
% just use channel 1
firstNonNanIndex = find(~isnan(adjLfpsCAR(1,:)), 1, 'first');
adjLfpsCAR(:,1:firstNonNanIndex-1) = [];
flashOnsets = flashOnsets - (firstNonNanIndex - 1)/Fs;
lastNonNanIndex = find(~isnan(adjLfpsCAR(1,:)), 1, 'last');
adjLfpsCAR(:,lastNonNanIndex+1:end) = [];
...
for j = 1:nChannels
    ...
    badIndices = adjLfpsCAR(j,:) > meanAdjLfp + outlierMaxSDStep1 * sdAdjLfp | ...
    ...
    if sum(badIndices) > 0
        fprintf('\tDetected %d outlier time points because voltage in trial is too extreme (> %0.1f SDs). Zeroing them out.\n', ...
                sum(badIndices), outlierMaxSDStep1);
    end
    ...
    adjLfpsCAR(j,badIndices) = meanAdjLfp; 
end
...
adjLfpsCAR(isnan(adjLfpsCAR)) = 0;
D.adjLfpsLP = filtfilt(bFirLowPassCustom, 1, adjLfpsCAR')';
% clear adjLfpsCAR;

%% temp
% for j = 1:nChannels, figure; plot(D.adjLfpsLP(j,:)); end

%% detect events with outlier activity (e.g. amp saturated)
outlierCheckStartIndices = round((flashOnsets + outlierCheckWindowOffset(1)) * Fs); 
...
for j = 1:nChannels
    ...
    channelDataNorm(j,:) = (D.adjLfpsLP(j,:) - nanmean(D.adjLfpsLP(j,:))) / nanstd(D.adjLfpsLP(j,:));
...
fprintf('Detected %d outlier flashes out of %d. Removing...\n', sum(isFlashEventOutlier), nFlashes);
flashOnsets(isFlashEventOutlier == 1) = [];
flashStats(isFlashEventOutlier == 1,:) = [];
nFlashes = numel(flashOnsets);
assert(nFlashes == size(flashStats, 1));
