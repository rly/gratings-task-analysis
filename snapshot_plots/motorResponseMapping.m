...
%% setup and load data
fprintf('\n-------------------------------------------------------\n');
fprintf('Juice Response Mapping Analysis - Spikes\n');
fprintf('Loading %s...\n', pl2FilePath);
...
origJuiceEvents = D.events{8};
nJuiceEvents = numel(origJuiceEvents);

%% SPDF params

psthWindow = [0.25 0.3]; % secs before, secs after

analysisWindowOffset = [0 0.2]; % secs offset from event onset
...
origJuiceEventsClean = origJuiceEvents;
sparseBlocks = find(~isFiringNonsparseByBlock(:,j));
rangeSparseRemovedJuiceEvents = nan(numel(sparseBlocks), 2); % start, end
for k = 1:numel(sparseBlocks)
    ...
    origJuiceEventsClean(origJuiceEventsClean >= blockStartTime & origJuiceEventsClean <= blockStopTime) = [];
    rangeSparseRemovedJuiceEvents(k,:) = [find(origJuiceEvents >= blockStartTime, 1, 'first') find(origJuiceEvents <= blockStopTime, 1, 'last')];
end

allAlignedSpikeTimes = createdatamatpt(spikeTimes, origJuiceEvents, psthWindow);
allAlignedSpikeTimesClean = createdatamatpt(spikeTimesClean, origJuiceEventsClean, psthWindow);
...
%% save ALL the values
D.allSpikeStructs{j}.juicePsthParams = var2struct(vepmIndices, psthWindow, ...
        analysisWindowOffset, baselineWindowOffset, ...
        latencyWindowOffset, kernelSigma, psthT, t, spikeTimesClean, ...
        origJuiceEventsClean, allAlignedSpikeTimes, allAlignedSpikeTimesClean, ...
        ...
        meanResponseByPsth, normPsthResponse, normMeanResponseByPsth, ...
        numRandomizations, bootstrappedMeanBaselines, ...
        responseMetricPValueByBootstrapPsth, minAbsNormMaxRespJuiceDriven);
...
if doPlot
    ...
    modTitle = sprintf('Juice-Locked Spiking: %s (%d juice events)', unitName, nJuiceEvents);
...
%         end
    ...
    for k = 1:numel(sparseBlocks)
        fillH = jbfill(xBounds, rangeSparseRemovedJuiceEvents(k,1) + [-0.5 -0.5], ...
                rangeSparseRemovedJuiceEvents(k,2) + [0.5 0.5], ...
                [1 0 0], ones(3, 1), 1, 0.1);
        uistack(fillH, 'bottom');
    end
    
    % shade in baseline and analysis windows
    fillH = jbfill(baselineWindowOffset, [0 0], [yBounds(2) yBounds(2)], [0.7 0.7 1], ones(3, 1), 1, 0.1);
    uistack(fillH, 'bottom');
    fillH = jbfill(analysisWindowOffset, [0 0], [yBounds(2) yBounds(2)], [0.3 1 0.3], ones(3, 1), 1, 0.1);
    uistack(fillH, 'bottom');
    ...
    if ~isempty(psthResponse)
        ...
        baselineShadingUB = baselineResponseByPsth + minAbsNormMaxRespJuiceDriven * baselineResponseSDOverTimeByPsth;
        baselineShadingLB = baselineResponseByPsth - minAbsNormMaxRespJuiceDriven * baselineResponseSDOverTimeByPsth;
        fillH = jbfill(xBounds, baselineShadingUB * ones(size(xBounds)), ...
                baselineShadingLB * ones(size(xBounds)), [1 0 1], ones(3, 1), 1, 0.08);
        uistack(fillH, 'bottom');
        ...
        fillH = jbfill(t, shadingUB, shadingLB, lines(1), ones(3, 1), 1, 0.2);
        uistack(fillH, 'bottom');
        ...
        fillH = jbfill(baselineWindowOffset, [yBounds(1) yBounds(1)], ...
                [yBounds(2) yBounds(2)], [0.7 0.7 1], ones(3, 1), 1, 0.1);
        uistack(fillH, 'bottom');
        fillH = jbfill(analysisWindowOffset, [yBounds(1) yBounds(1)], ...
                [yBounds(2) yBounds(2)], [0.3 1 0.3], ones(3, 1), 1, 0.1);
        uistack(fillH, 'bottom');
        hold on;

        xlim(xBounds);
        ylim(yBounds);
        xlabel('Time from juice onset (s)');
        ylabel('Estimated spike rate (Hz)');
    end
end

%% add response metrics text
D.allSpikeStructs{j}.isJuiceDriven = -1;
...
if ~isempty(psthResponse)
    ...
    if proportionFlashesWithSpike >= minProportionJuiceEventsWithSpikeJuiceDriven && ...
            (abs(normMaxResponseByPsth) >= minAbsNormMaxRespJuiceDriven || ...
            abs(normMinResponseByPsth) >= minAbsNormMaxRespJuiceDriven)
        visRespText = 'Juice Driven';
        visRespTextCol = [0.05 0.5 0.05];
        D.allSpikeStructs{j}.isJuiceDriven = 1;
    else
        visRespText = 'Not Juice Driven';
        visRespTextCol = [0.9 0.2 0.05];
        D.allSpikeStructs{j}.isJuiceDriven = 0;
    end
    
    %% compute latency
    minPeakForLatency = minAbsNormMaxRespJuiceDriven;
    maxTroughForLatency = -minAbsNormMaxRespJuiceDriven;
    
    if D.allSpikeStructs{j}.isJuiceDriven
        % check peak
    ...
    if doPlot
        ...
        text(axBig, -0.03, infoText2Top, {...
                ...
                '', ...
                sprintf('"Juice Driven": \\geq %d%% Flashes w/ Spike', round(minProportionJuiceEventsWithSpikeJuiceDriven * 100)), ...
                sprintf('  and Max or Min Response \\geq %0.1f SDs', minAbsNormMaxRespJuiceDriven), ...
...
%% save
if doPlot
    plotFileName = sprintf('%s/%s-%s-motorResponse.png', processedDataDir, unitName, blockName);
...
for j = 1:nUnits
    ...
    if any(isnan(spikeStruct.juicePsthParams.normPsthResponse)) || ...
            spikeStruct.juicePsthParams.meanResponseByPsth < minResponseByPsth
        continue;
    end
    
    plotCount = plotCount + 1;
    % reverse yVal since we reverse the axis
    yVal = -1 * spikeStruct.juicePsthParams.normPsthResponse * yScale + plotCount;
    plot(spikeStruct.juicePsthParams.t, yVal, '-', 'Color', col);
    fillX = [spikeStruct.juicePsthParams.t spikeStruct.juicePsthParams.t([end 1])];
    fillY = [yVal plotCount plotCount];
    fillC = [spikeStruct.juicePsthParams.normPsthResponse 0 0];
...
for j = 1:nUnits
    ...
    % reverse yVal since we reverse the axis
    yVal = -1 * spikeStruct.juicePsthParams.normPsthResponse * yScale + plotCount;
    plot(spikeStruct.juicePsthParams.t, yVal, '-', 'Color', col, 'LineWidth', 1);
%     fillX = [spikeStruct.juicePsthParams.t spikeStruct.juicePsthParams.t([end 1])];
%     fillY = [yVal plotCount plotCount];
%     fillC = [spikeStruct.juicePsthParams.normPsthResponse 0 0];
%     fill(fillX, fillY, fillC, 'FaceAlpha', 0.85);
...
for j = 1:nUnits
    ...
    D2.allSpikeStructs{j}.isJuiceDriven = D.allSpikeStructs{j}.isJuiceDriven;
    ...
    D2.allSpikeStructs{j}.juicePsthParams = D.allSpikeStructs{j}.juicePsthParams;
end
D = D2;
saveFileName = sprintf('%s/D-all-withJuiceDrivenTag.mat', processedDataDir);
fprintf('Writing post-processed data file: %s ...', saveFileName);
tic;
save(saveFileName, 'D');
fprintf(' done (%0.2f s).\n', toc);