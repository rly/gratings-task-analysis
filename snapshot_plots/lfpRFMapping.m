...
flashStatsClean = flashStats(~isEventOutlier,:);
...
responseMetric = zeros(numel(distsToFixUnique), numel(polarAnglesUnique), numel(gratingAnglesUnique)+1);
baselineMetric = zeros(numel(distsToFixUnique), numel(polarAnglesUnique), numel(gratingAnglesUnique)+1);
trialCounts = zeros(numel(distsToFixUnique), numel(polarAnglesUnique), numel(gratingAnglesUnique));
...
%             averageFlashResponseMeanOverTime(k,l,m) = mean(abs(averageFlashResponse(k,l,m,:)));
        ...
        theseFlashOnsets = flashOnsetsClean(flashStatsClean(:,4) == k & flashStatsClean(:,5) == l);
        if isempty(theseFlashOnsets)
            continue;
        end
...
rfmapAllOri = squeeze(sum(rfmapByOri(:,numel(gratingAnglesUnique)+1,:,:), 1));
rfmapAllOriCount = squeeze(sum(rfmapByOriCount(:,numel(gratingAnglesUnique)+1,:,:), 1));
...
rfmapAllSmoothByOriAll = squeeze(sum(rfmapSmoothByOri(:,numel(gratingAnglesUnique)+1,:,:), 1));
